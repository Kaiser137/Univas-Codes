public class FilaDinamica {
    ...
    public int batataQuente(FilaDinamica roda, int passes){
        if (roda == null){
            return 0;
        }
        
        int tamanhoDaRoda = roda.tamanho;

        while(tamanhoDaRoda > 1){
            for (int i = 0; i < passes; i++){
                int pessoa = roda.desenfileirar();
                roda.enfileirar(pessoa);
            }
            roda.desenfileirar();
            tamanhoDaRoda--;
        }
        
        return roda.observar();
    }
}